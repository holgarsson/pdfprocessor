...
public class FinancialData
{
    ...
    /// <summary>
    /// Normalizes financial data by dividing all non-null decimal values by 1000 if not already in thousands
    /// </summary>
    public void NormalizeFinancialData()
    {
        ...
        foreach (var property in properties)
        {
            ...
            if (value.HasValue)
            {
                property.SetValue(this, Math.Round(value.Value / 1000m, 3, MidpointRounding.AwayFromZero));
            }
        }
    ...
    public static FinancialData FromJson(string jsonString)
    {
        ...
        foreach (JsonProperty property in jsonDocument.RootElement.EnumerateObject())
        {
            ...
            try
            {
                ...
                // Handle numeric values that come as strings (e.g. "3.158.905" or "-3.775.011")
                if (property.Value.ValueKind == JsonValueKind.String)
                {
                    ...
                    if (decimal.TryParse(processedValue, System.Globalization.NumberStyles.Any, System.Globalization.CultureInfo.InvariantCulture, out decimal decimalValue))
                    {
                ...
                if (property.Value.ValueKind == JsonValueKind.Number)
                {
                    ...
                    {
                        Console.WriteLine($"Error parsing number for property {property.Name}: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
